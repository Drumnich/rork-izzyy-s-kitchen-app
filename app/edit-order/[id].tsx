...
export default function EditOrderScreen() {
  ...
  useEffect(() => {
    if (order) {
      const matchingCustomer = customers.find(c => c.name === order.customerName);
      if (matchingCustomer) {
      ...
      setSelectedDay(deadline.getDate());
      setSelectedHour(deadline.getHours());
  ...
  useEffect(() => {
    if (showProductPicker) {
      console.log('🧁 EditOrder - Refreshing recipes for product picker');
      loadRecipes();
    } else {
  ...
  const handleUpdateOrder = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Order Updated!', 
        `Order for ${customerName} has been successfully updated.`,
        [
          { 
            text: 'OK', 
            onPress: () => router.back()
